...
with open('spa.txt', 'r') as f:
    ...
    for line in f:
        ...
        # remove Spanish punctuations
        sp_words = list(map(lambda x: pattern.sub('', x), output_text.split()))
        sp_words = ['\t']+sp_words+['\n']
        sp_vocs.update(sp_words)
        input_texts.append(en_words)
        output_texts.append(sp_words)
        cnt += 1
        if cnt >= num_samples:
            break
...
sp_to_int, int_to_sp = {w: i+1 for i, w in enumerate(sp_vocs)}, {i+1: w for i, w in enumerate(sp_vocs)}

num_encoder_tokens = len(en_vocs)+1
num_decoder_tokens = len(sp_vocs)+1
...
int_output_data = [[sp_to_int[w] for w in row] for row in output_texts]
